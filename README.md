...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other RUNEBASE wallets.

It can sign transactions locally, and submit the raw transaction data to a remote runebase node. The blockchain data is provided by the Insight API (which powers https://explorer.runebase.org/), rather than the raw runebased RPC calls.

This library makes it possible to run DApp without the users having to run a full runebased node.

> This library is extracted from the official [RUNEBASE web wallet](https://github.com/runebase/runebase-web-wallet).

## Install

```
yarn add runebasejs-wallet
```

## Running Tests

The runebasejs-wallet depends on a number of external services. To run the unit tests, you'll need to start the services using [docker-compose](https://docs.docker.com/compose/install/).
...
- Uses satoshi (1e8) as internal units
  - Can represent up to ~90 million RUNEBASE accurately.
...
```js
import { networks, generateMnemonic } from "runebasejs-wallet"
...
The currency unit used is `satoshi`. To convert runebase to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "runebasejs-wallet"
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 runebase, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8, // 0.05 runebase in satoshi
  })
...
Suppose you want to import the public address `qg3HYD8c4bAVLeEzA9t3Ken3Y3Mni1HZSS`. Use `runebase-cli` to dump the private key from wallet:
...
const amount = 0.15 * 1e8 // 0.15 RUNEBASE
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 runebase/KB, as is typical.
  feeRate: 400,
})
```
...
You can decode the raw transaction using `runebase-cli`:

```
runebase-cli decoderawtransaction 0100000001a09a921ecb38...
...
```
runebase-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
...
```js
import { scrypt } from "runebasejs-wallet"
...
```js
import { scrypt, params } from "runebasejs-wallet"
